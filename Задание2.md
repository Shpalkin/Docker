...
2. Запустите контейнер node в интерактивном режиме подключения терминала, поименуйте его `mynode`, передайте две переменные среды `NAME=<ваше имя>` и `SURNAME=<ваша фамилия>`;\
    Решение: Выполнить команду `docker run  --name mynode -it -e NAME=Aleksey -e SURNAME=Dmitriev node:15.14`;\
    Вывод консоли:
```console
PS ndse-homeworks_009-docker> docker run  --name mynode -it -e NAME=Aleksey -e SURNAME=Dmitriev node:15.14
...
3. В интерактивной среде выполнения node выполните скрипт, который выведет на экран приветсвтие: `Привет, <ваше имя> <ваша фамилия>!`, эти данные должны быть получены из переменных среды;\
    Решение: Выполнить команду `console.log("Привет, " + process.env.NAME + " " + process.env.SURNAME + "!");`;\
    Вывод консоли:
```console
PS ndse-homeworks_009-docker> docker run  --name mynode -it -e NAME=Aleksey -e SURNAME=Dmitriev node:15.14
Welcome to Node.js v15.14.0.      
...
Привет, Aleksey Dmitriev!
undefined
>
```
4. Остановите контейнер;\
    Решение: Выполнить команду `docker stop mynode`;\
    Вывод консоли:
```console
> .exit
PS ndse-homeworks_009-docker> docker stop mynode
mynode
```
5. Удалите образ node версии 15.14;\
    Решение: Выполнить команды `docker rm mynode` `docker rmi node:15.14`;\
    Вывод консоли:
```console
PS ndse-homeworks_009-docker> docker rm mynode
mynode
